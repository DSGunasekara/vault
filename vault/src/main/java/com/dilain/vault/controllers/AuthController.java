...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<String>> login(@Valid @RequestBody LoginRequest request) {
        ...
        if (user.isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(new ApiResponse<String>(ResponseStatus.ERROR, "Username is not found", null));
        }
    ...
    public ResponseEntity<ApiResponse<UserDto>> register(@Valid @RequestBody RegisterRequest request) {
        if (userService.findByUsername(request.username()).isPresent()) {
            return ResponseEntity.badRequest()
                    .body(new ApiResponse<>(ResponseStatus.ERROR, "Username already taken", null));
        }

        User user = userService.createUser(request);

        return ResponseEntity
                .ok(new ApiResponse<>(ResponseStatus.SUCCESS, "User registered successfully", UserDto.from(user)));
    }
}